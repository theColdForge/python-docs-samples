...
def create_topics() -> None:
    ...
    for _ in range(1, 10):
        try:
            ...
            topics = []
            topics.append(
                NewTopic(name=TOPIC_NAMES[0], num_partitions=1, replication_factor=1)
            )
            topics.append(
                NewTopic(name=TOPIC_NAMES[1], num_partitions=1, replication_factor=1)
            )
...
def test_read_kafka(
    docker_client: DockerClient, tmp_path: Path, file_name_prefix: str
) -> None:
    topic = TOPIC_NAMES[0]
...
def test_read_kafka_multi_topic(
    docker_client: DockerClient, tmp_path: Path, file_name_prefix: str
) -> None:
    # Run the containerized Dataflow pipeline.
    docker_client.containers.run(
    ...
    for topic in TOPIC_NAMES:
        verify_output(
            f"{tmp_path}/{file_name_prefix}/{topic}/output-00000-of-00001.txt", topic
        )
